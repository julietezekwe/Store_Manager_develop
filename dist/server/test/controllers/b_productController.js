...
var emptyField = _testData.productDetails.emptyField,
    ...
    updateCategory = _testData.productDetails.updateCategory;

describe('Products Endpoint API Test', function () {
  ...
  it('it should not update product if product does not exist', function (done) {
  ...
  it('it should update product category ', function (done) {
    _chai2.default.request(_app2.default).put('/api/v1/products/1/category').set('Authorization', authToken2).send(updateCategory).end(function (err, res) {
      expect(res.body.message).to.eql('Successfully updated product category');
      expect(res.status).to.equal(201);
      done();
    });
  });
  it('it should not update product category if product does not exist', function (done) {
    _chai2.default.request(_app2.default).put('/api/v1/products/40/category').set('Authorization', authToken).send(updateCategory).end(function (err, res) {
      expect(res.body.message).to.eql('Product does not exist');
      expect(res.status).to.equal(404);
      done();
    });
